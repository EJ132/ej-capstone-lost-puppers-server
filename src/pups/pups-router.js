...
PupsRouter
  ...
  .post("/", upload.single("image"), (req, res, next) => {
    ...
    PupsService.insertPup(req.app.get("db"), newPup)
      .then((Pup) => {
        res.status(201).json(PupsService.serializePup(Pup));
      })
...
PupsRouter.route("/:id")
  ...
  .delete((req, res, next) => {
    PupsService.deletePup(req.app.get("db"), req.params.id)
  ...
  .patch(jsonBodyParser, (req, res, next) => {
    ...
    PupsService.updatePup(req.app.get("db"), req.params.id, pupToUpdate)
      .then((updatedPup) => {
...
PupsRouter.route("/:id/comments")
  .all(requireAuth)
  .get((req, res, next) => {
    PupsService.getAllComments(req.app.get("db"), req.params.id)
...
async function checkPupExists(req, res, next) {
  try {
    const thing = await PupsService.getById(req.app.get("db"), req.params.id);
